...
クマのキャラクターのNFTプロダクト "KUMALEON" の発表時のプロモーション案件として作成した映像です。
KUMALEON自身はアニメーションを付けないという制約があったため、カメラや周りのオブジェクトの動きで情報量を調節するよう心がけました。
タイトなスケジュールの中で レンダリング ↔ 調整 の試行回数を増やすため、Karmaレンダラで大量のジオメトリをインスタンスとして高速にレンダリングできるよう工夫しました。
